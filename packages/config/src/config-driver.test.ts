import { getConfigRecords } from './config-driver'
...
describe('env driver', () => {
    it('can load variables from environment', async () => {
        ...
        const envDriver = await getConfigRecords('env://TEST_ENV')
    ...
    it('can alias env variables', async () => {
        process.env.TEST_ENV = 'TestVal'
        const envDriver = await getConfigRecords('env://TEST_ENV:testEnv')
...
describe('json driver', () => {
    it('can load variables from json file', async () => {
        ...
        try {
            ...
            const jsonDriver = await getConfigRecords(`json://${configFile}`)
